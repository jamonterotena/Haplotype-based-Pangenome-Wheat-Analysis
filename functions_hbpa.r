...
COV <- function(data = data.frame(), chr_length = table_cultivar_chr_length, cultivar_names = c("arinalrfor", "jagger", "julius", "lancer", "landmark", "mace", "norin61", "stanley", "sy_mattis", "chinese"), scaffold_level_assemblies = c("claire", "cadenza", "paragon", "robigus", "weebil")){
  all_scaffold_level_assemblies = c("claire", "cadenza", "paragon", "robigus", "weebil")
  cov_cultivar_CLA_CLA <- vector()
  cov_cultivar_CLA_SLA <- vector()
  for (i in 1:length(cultivar_names)){
    to_add_CLA_CLA <- sum(data$r_length[grepl(paste("^", cultivar_names[i], sep = ""), data$comparison) & !grepl(paste(all_scaffold_level_assemblies, collapse = "|"), data$comparison) & !grepl("spelta", data$comparison)])/(length(cultivar_names)-1)
    to_add_CLA_SLA <- sum(data$r_length[grepl(paste("^", cultivar_names[i], sep = ""), data$comparison) & grepl(paste(scaffold_level_assemblies, collapse = "|"), data$comparison) & !grepl("spelta", data$comparison)])/length(scaffold_level_assemblies)
    to_add_CLA_CLA <- to_add_CLA_CLA/chr_length$sequence_length[grepl(paste0("chr", data$chrom[1], collapse = ""), chr_length$sequence_name) & grepl(cultivar_names[i], chr_length$cultivar_name)]
    to_add_CLA_SLA <- to_add_CLA_SLA/chr_length$sequence_length[grepl(paste0("chr", data$chrom[1], collapse = ""), chr_length$sequence_name) & grepl(cultivar_names[i], chr_length$cultivar_name)]
    cov_cultivar_CLA_CLA <- append(cov_cultivar_CLA_CLA, to_add_CLA_CLA*100)
    cov_cultivar_CLA_SLA <- append(cov_cultivar_CLA_SLA, to_add_CLA_SLA*100)
  }
  names(cov_cultivar_CLA_CLA) <- cultivar_names
  names(cov_cultivar_CLA_SLA) <- cultivar_names
  mean_cov_cultivar_CLA_CLA <- mean(cov_cultivar_CLA_CLA)
  mean_cov_cultivar_CLA_SLA <- mean(cov_cultivar_CLA_SLA)
  output <- list('Average % coverage with CHROMOSOME-LEVEL ASSEMBLIES as query' = mean_cov_cultivar_CLA_CLA,
                 '(By cultivar)' = cov_cultivar_CLA_CLA,
                 'Average % coverage with SCAFFOLD-LEVEL ASSEMBLIES as query' = mean_cov_cultivar_CLA_SLA,
                 '(By cultivar)' = cov_cultivar_CLA_SLA)
  return(output)
}

LENGTH <- function(data = data.frame(), cultivar_names = c("arinalrfor", "jagger", "julius", "lancer", "landmark", "mace", "norin61", "stanley", "sy_mattis", "chinese"), scaffold_level_assemblies = c("claire", "cadenza", "paragon", "robigus", "weebil")){
  all_scaffold_level_assemblies = c("claire", "cadenza", "paragon", "robigus", "weebil")
  length_cultivar_CLA_CLA <- vector()
  length_cultivar_CLA_SLA <- vector()
  for (i in 1:length(cultivar_names)){
    to_add_CLA_CLA <- mean(data$r_length[grepl(paste("^", cultivar_names[i], sep = ""), data$comparison) & !grepl(paste(all_scaffold_level_assemblies, collapse = "|"), data$comparison) & !grepl("spelta", data$comparison)])
    to_add_CLA_SLA <- mean(data$r_length[grepl(paste("^", cultivar_names[i], sep = ""), data$comparison) & grepl(paste(scaffold_level_assemblies, collapse = "|"), data$comparison) & !grepl("spelta", data$comparison)])
    length_cultivar_CLA_CLA <- append(length_cultivar_CLA_CLA, to_add_CLA_CLA)
    length_cultivar_CLA_SLA <- append(length_cultivar_CLA_SLA, to_add_CLA_SLA)
  }
  names(length_cultivar_CLA_CLA) <- cultivar_names
  names(length_cultivar_CLA_SLA) <- cultivar_names
  mean_length_cultivar_CLA_CLA <- mean(length_cultivar_CLA_CLA)
  mean_length_cultivar_CLA_SLA <- mean(length_cultivar_CLA_SLA)
  output <- list('Average aln length with CHROMOSOME-LEVEL ASSEMBLIES as query' = mean_length_cultivar_CLA_CLA,
                 '(By cultivar)' = length_cultivar_CLA_CLA,
                 'Average aln length with SCAFFOLD-LEVEL ASSEMBLIES as query' = mean_length_cultivar_CLA_SLA,
                 '(By cultivar)' = length_cultivar_CLA_SLA)
  return(output)
}

NUMBER <- function(data = data.frame(), cultivar_names = c("arinalrfor", "jagger", "julius", "lancer", "landmark", "mace", "norin61", "stanley", "sy_mattis", "chinese"), scaffold_level_assemblies = c("claire", "cadenza", "paragon", "robigus", "weebil")){
  all_scaffold_level_assemblies = c("claire", "cadenza", "paragon", "robigus", "weebil")
  number_cultivar_CLA_CLA <- vector()
  number_cultivar_CLA_SLA <- vector()
  for (i in 1:length(cultivar_names)){
    to_add_CLA_CLA <- nrow(data[grepl(paste("^", cultivar_names[i], sep = ""), data$comparison) & !grepl(paste(all_scaffold_level_assemblies, collapse = "|"), data$comparison) & !grepl("spelta", data$comparison),])/(length(cultivar_names)-1)
    to_add_CLA_SLA <- nrow(data[grepl(paste("^", cultivar_names[i], sep = ""), data$comparison) & grepl(paste(scaffold_level_assemblies, collapse = "|"), data$comparison) & !grepl("spelta", data$comparison),])/length(scaffold_level_assemblies)
    number_cultivar_CLA_CLA <- append(number_cultivar_CLA_CLA, to_add_CLA_CLA)
    number_cultivar_CLA_SLA <- append(number_cultivar_CLA_SLA, to_add_CLA_SLA)
  }
  names(number_cultivar_CLA_CLA) <- cultivar_names
  names(number_cultivar_CLA_SLA) <- cultivar_names
  mean_number_cultivar_CLA_CLA <- mean(number_cultivar_CLA_CLA)
  mean_number_cultivar_CLA_SLA <- mean(number_cultivar_CLA_SLA)
  output <- list('Average aln number with CHROMOSOME-LEVEL ASSEMBLIES as query' = mean_number_cultivar_CLA_CLA,
                 '(By cultivar)' = number_cultivar_CLA_CLA,
                 'Average aln number with SCAFFOLD-LEVEL ASSEMBLIES as query' = mean_number_cultivar_CLA_SLA,
                 '(By cultivar)' = number_cultivar_CLA_SLA)
  return(output)
}
...
plot_line_bin_median <- function(data, bin_start = 0, bin_end, bin_size = 1e+07, cut_off = 99.99, ymin = 97, ymax = 100, reference_name = data$rid, query_name = data$qid, x_label_gap){
  ...
  ggplot(comparison_medians, aes(x=bin, y = perc_id_median, colour = cut_off)) +
    ...
    ggtitle(paste0("Median line: ", reference_name, " vs ", query_name, ", zoom region: ", bin_start, "-", bin_end, " Mbp", ", bin size: ", bin_size/1000000, "-Mbp"))
}
...
plot_aln_and_bins <- function(aln_subset = data.frame(), bin_size = bin_size, zoom_start = zoom_start, zoom_end = zoom_end, highlighted_target = target, target_text = "target region", fill_target = "orange", color_target_text = "black", fill_predictions = "green", color_prediction_text = "black",  ymin = 98.5, cut_off = 99.99, reference_name = reference_cultivar, query_name = query_cultivar, dot_size = 4, x_label_gap = 1000000, print_tables = TRUE, prediction_text = TRUE, aln_text = TRUE){
  medians_zoom <- plot_line_bin_median(data = aln_subset, bin_size = bin_size, bin_start = zoom_start, bin_end = zoom_end, ymin = ymin, cut_off = cut_off, reference_name = reference_cultivar, query_name = query_cultivar, x_label_gap = x_label_gap)
  ...
  medians_zoom_block_sum <- block_summary(medians_zoom_bin_info, bin_size = bin_size, show_only_coords = FALSE, reference_name = reference_cultivar, query_name = query_cultivar)
  if (print_tables) { 
  ...
  medians_zoom_block_coords <- block_summary(medians_zoom_bin_info, bin_size = bin_size, show_only_coords = TRUE, reference_name = reference_cultivar, query_name = query_cultivar)
  ...
  if(nrow(medians_zoom_block_coords) == 0){
    ...
    coords <- block_summary(medians_zoom_bin_info, bin_size = bin_size, show_only_coords = TRUE, reference_name = reference_cultivar, query_name = query_cultivar)
  }
  graph <- plot_aln_pid_and_length(aln_subset, xmin = zoom_start, ymin = ymin, xmax = zoom_end, reference_name = reference_cultivar, query_name = query_cultivar , x_label_gap = x_label_gap, dot_size = dot_size)
...
plot_bins_and_selected_region <- function(aln_subset = data.frame(), bin_size = bin_size, zoom_start = zoom_start, zoom_end = zoom_end, highlighted_target = target, target_text = "target region", fill_target = "orange", color_target_text = "black", fill_predictions = "green", color_prediction_text = "black",  ymin = 98.5, cut_off = 99.99, reference_name = reference_cultivar, query_name = query_cultivar, dot_size = 4, x_label_gap = 1000000, print_tables = TRUE, prediction_text = TRUE){
  medians_zoom <- plot_line_bin_median(data = aln_subset, bin_size = bin_size, bin_start = zoom_start, bin_end = zoom_end, ymin = ymin, cut_off = cut_off, reference_name = reference_cultivar, query_name = query_cultivar, x_label_gap = x_label_gap)
  medians_zoom_bin_info <- assign_blocks_mummer(medians_zoom[["data"]], original_file = aln_subset)
  aln_number_per_bin <- medians_zoom_bin_info$aln_number[2:length(medians_zoom_bin_info$aln_number)]
  mid_point <- (medians_zoom_bin_info$bin_start[2:length(medians_zoom_bin_info$aln_number)]+medians_zoom_bin_info$bin_end[2:length(medians_zoom_bin_info$aln_number)])/2
  ...
  graph <- plot_line_bin_median(data = aln_subset, bin_start = zoom_start, bin_end = zoom_end, bin_size = bin_size, cut_off = cut_off, ymin = ymin, ymax = 100, reference_name = reference_cultivar, query_name = query_cultivar, x_label_gap = x_label_gap)
  if (is.data.frame(highlighted_target) == FALSE){
